...
//         {
//             MapPoint *pMP = *vit;//!从地图点集合中选择一个点
//             if (pMP)
//                 if (!pMP->isBad() && pMP->GetMap() == pCurrentMap)//!保证地图点有效
//                 {

//                     if (pMP->mnBALocalForKF != pKF->mnId) //!防止地图点的重复添加
//                     {
//                         lLocalMapPoints.push_back(pMP); //!将地图点放到lLocalMapPoints里面
...
//     // if (num_fixedKF < 2)
//     // {
//     //     std::cout << "程序运行到了Optimizer.cc中的第1876行" << std::endl;
...
//     //!添加不优化的位姿顶点，为什么不优化也要添加呢，为了增加约束信息
...
//         vPoint->setEstimate(pMP->GetWorldPos().cast<double>());//!地图点的世界坐标系坐标设为初值
//         int id = pMP->mnId + maxKFid + 1; //!在maxKFid 之后再设置地图点的ID
//         vPoint->setId(id);
//         //!由于使用了LinearSolverType， 所以需要将所有的三维点边缘化掉
//         // 这里的边缘化与滑动窗口不同，而是为了加速稀疏矩阵的计算BlockSolver_6_3默认了6维度的不边缘化，3自由度的三维点被边缘化，所以所有三维点都设置边缘化
//         vPoint->setMarginalized(true);
//         optimizer.addVertex(vPoint);
//         nPoints++;
//         //观测到该地图点的关键帧，和该地图点在关键帧中的索引
...
//                 {
//                     const cv::KeyPoint &kpUn = pKFi->mvKeysUn[leftIndex];//!当前地图点对应的二维的特征点
//                     Eigen::Matrix<double, 2, 1> obs;
...
//                     e->setMeasurement(obs);
//                     //!边的权重的设置方式（信息矩阵）
//                     //!权重为特征点所在的图像金字塔的层数的倒数，层数越高，该特征点的误差置信度越低
...
//     //!删除点，在关键帧中剔除对该地图点的观测，在地图点中剔除对该关键帧的观测
//     if (!vToErase.empty())
//     {
...
//! ********************************************************
int Optimizer::PoseOptimizationOnlyforRGBD(Frame* pFrame){
    // 该优化函数主要用于Tracking线程中：运动跟踪、参考帧跟踪、地图跟踪、重定位
    ...
    vector<ORB_SLAM3::EdgeSE3XYZRGBDOnlyPose *> vpEdgesMono;           // 存放单目边
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesMono_FHR; // 存放另一目的边
    vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight;                     // 边对应特征点的id
    vpEdgesMono.reserve(N);
    vpEdgesMono_FHR.reserve(N);
    ...
    // Step 3：添加一元边
    {
        ...
        {
                /* 
                观察视角是描述地图点在关键帧中的观察情况的一组参数，
                包括该地图点在关键帧中的像素坐标、特征点的描述子以及该关键帧的相机位姿等信息。
                通过存储每个地图点在不同关键帧下的观察视角，
                ORB-SLAM 可以通过特征点匹配和三角化等方法来重建地图，并优化地图点的位置和状态。
                需要注意的是，mvpMapPoints 中存储的观察视角是针对每个地图点的，
                而不是针对相机的。每个观察视角都是由一个关键帧和该关键帧下的特征点索引组成的。
                因此，通过 mvpMapPoints 中的信息，可以确定每个地图点在不同关键帧下的观察情况，
                从而进行后续的地图管理和优化操作。*/

            MapPoint *pMP = pFrame->mvpMapPoints[i];

            //!注意此处我可以这样获得地图点的观测值
            //! float a = pMP->mTrackProjX;
            // 如果这个地图点还存在没有被剔除掉
            if (pMP)
            {
                // Conventional SLAM
                ...
                if (!pFrame->mpCamera2)
                {
                    // Monocular observation
                    {
                        ...
                        //!这里准备把他改成三维空间的，z的值也就是深度值从哪里来？
                        Eigen::Matrix<double, 3, 1> obs;
                        const cv::KeyPoint &kpUn = pFrame->mvKeysUn[i];
                        const float &depthZ = pFrame->mvDepth[i];
                        obs << kpUn.pt.x, kpUn.pt.y, depthZ;

                        ORB_SLAM3::EdgeSE3XYZRGBDOnlyPose *e = new ORB_SLAM3::EdgeSE3XYZRGBDOnlyPose(
                            Eigen::Vector3d(pMP->mTrackProjX,pMP->mTrackProjY,pMP->mTrackDepth));
    ...
    for (size_t it = 0; it < 4; it++)
    {
        ...
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++)
        {
            ORB_SLAM3::EdgeSE3XYZRGBDOnlyPose *e = vpEdgesMono[i];
 ...
 * 前面操作基本一样，但优化时2代去掉了误差大的点又进行优化了，3代只是统计但没有去掉继续优化，而后都是将误差大的点干掉
 */
void Optimizer::LocalBundleAdjustmentOnlyForRGBD(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
{
    // 该优化函数用于LocalMapping线程的局部BA优化
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame *> lLocalKeyFrames;
